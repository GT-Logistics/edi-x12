...
use Gtlogistics\EdiX12\Test\Factory\EdiFactory;

class EdiTestCase extends TestCase
{
    private EdiFactory $ediFactory;

    protected function setUp(): void
    {
        $this->ediFactory = new EdiFactory();
    }

    protected function tearDown(): void
    {
        unset($this->ediFactory);
    }

    protected function createEdi204(): Edi
    {
        return $this->ediFactory->make204();
    }

    protected function createEdi997(): Edi
    {
        return $this->ediFactory->make997();
    }

    protected function assertEdi204(Edi $value): void
    {
        $this->assertCount(1, $value->ISA);
        $isa = $value->ISA[0];
        ...
        $nte = $st->NTE[0];
        $this->assertInstanceOf(NTESegment::class, $nte);
        $this->assertNull($nte->_01);
        $this->assertSame('Lorem ipsum dolor sit amet, consectetur adipiscing elit.', $nte->_02);

        $nte = $st->NTE[1];
        $this->assertInstanceOf(NTESegment::class, $nte);
        $this->assertNull($nte->_01);
        $this->assertSame('Proin porta lectus eu faucibus aliquet.', $nte->_02);

        $nte = $st->NTE[2];
        $this->assertInstanceOf(NTESegment::class, $nte);
        $this->assertNull($nte->_01);
        $this->assertSame('Nam pretium justo interdum, ultrices odio ac, blandit nulla.', $nte->_02);
    ...
    protected function assertX12(string $expected, string $value): void
    {
